    ...
    try {
        jsonwebtoken.verify(token, process.env.SECRET_KEY, async (err, user) => {
            ...
            if (req.user.role !== "user") {
                res.status(401).json({ message: "You are not authorized to assign a task, mortal" })
            }
            const assignedTask = await Task.findByIdAndUpdate(req.params.taskId, { updated_at: new Date(), updated_by: req.user.id, responsible_user: req.params.userId })
            res.status(201).json({ success: true, message: `Task ${assignedTask._id} assigned successfully to user ${assignedTask.responsible_user}. Congrats` })
        })
